...
def dc_proxy(defender: Dict[str, Any], state: Dict[str, Any]) -> float:
    ...
    rule_cost = clamp(
        (
            safe_float(overhead.get("flow_rules_installed"))
            + safe_float(overhead.get("flow_delete_commands"))
            + safe_float(overhead.get("meters_added"))
        )
        / 120.0
    )
    service_cost = clamp(
...
def normalize_population(ids: List[str], previous: Optional[Dict[str, float]] = None) -> Dict[str, float]:
    if not ids:
        return {}
    previous = previous or {}
    uniform = 1.0 / len(ids)
    values = {sid: max(safe_float(previous.get(sid), uniform), 0.000001) for sid in ids}
    total = sum(values.values())
    if total <= 0:
        return {sid: uniform for sid in ids}
    return {sid: value / total for sid, value in values.items()}
...
def replicator_update(
    ...
    for sid, share in population.items():
        ...
        updated[sid] = max(next_share, 0.000001)
    total = sum(updated.values())
    return {sid: value / total for sid, value in updated.items()}, average
