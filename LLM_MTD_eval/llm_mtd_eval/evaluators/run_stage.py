...
from ..emo_bridge import load_emo_strategy_modules, strategy_dir
from ..llm_layer.defender_selector import select_baseline_top_defender, select_defender_strategy
from ..llm_layer.stage_summarizer import build_stage_summary_record
from ..reports.export_json import write_json
...
def _read_latest_stage_memory(path: Path, scenario_id: str) -> dict[str, Any]:
    if not path.exists():
        return {}
    try:
    ...
    for line in reversed(lines):
        ...
        return {
            ...
            "previous_defender_strategy_id": ((selection.get("defender") or {}).get("id") or ""),
...
def _canonical_stage_summary_text(
    *,
    ...
    if security_outcome["defense_success"]:
    ...
    elif not security_outcome["defense_confirmed"]:
        outcome = "defense was not confirmed by controller evidence"
    else:
...
def build_stage_outcome(
    *,
    ...
    if next_state is not None:
        current_state = merge_confirmed_defense_effects_into_state(next_state, effect_validation)
    else:
    ...
    outcome_consistent = bool(
        attack_effect_success == bool(current_state.get("attack_effect_success"))
        and (not (attack_effect_success and defense_success))
    )
    ...
    validation = {
        ...
        "llm_request_error": defender_result.request_error,
        "llm_parse_success": bool(defender_result.parse_success),
        "llm_recovery_used": bool(defender_result.recovery_used),
...
def run_stage(
    *,
    ...
    modules = load_emo_strategy_modules(config.workspace_root)

    emo_strategy_dir = strategy_dir(config.workspace_root)
    ...
    previous_stage_memory = _read_latest_stage_memory(stage_log_path, str(stage_state.get("scenario_id", scenario_id)))
    ...
    llm_metadata = {
        ...
        "llm_ranked_candidates": defender_result.ranked_candidates,
        **alignment,
        "llm_override_reason": defender_result.override_reason,
        "llm_urgency_level": defender_result.urgency_level,
