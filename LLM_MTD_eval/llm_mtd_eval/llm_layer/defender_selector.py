...
from ..types import LLMResponseTrace
...
def _build_prompts(payload: dict[str, Any]) -> tuple[str, str]:
    if str(payload.get("prompt_mode") or "full") == "compact":
        ...
        system_prompt = (
            ...
            "one defender strategy from the allowed active defender candidates. Balance current verified threat "
            "evidence, path-stage urgency, expected security gain, QoS impact, controller cost, previous failed "
            "defenses, telemetry confidence, and state freshness. Attack dispatched alone is not attack active; "
            "path relevance alone is not proof of ongoing compromise; zero workload plus source errors means the "
            "state may be stale or partially unavailable. Distinguish reactive_containment, proactive_hardening, "
            "and uncertainty_limited responses. Prefer the least disruptive sufficient action when threat evidence "
            "is weak, stale, or contradictory. Use previous-stage memory as supporting context, not as the sole "
            "reason for aggressive disruption. Return valid JSON only with the required schema."
        )
