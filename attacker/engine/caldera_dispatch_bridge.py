...
DEFAULT_SCENARIO_REGISTRY = Path(__file__).resolve().parents[1] / "scenarios" / "attack_scenarios.json"
...
def _adversary_has_abilities(adversary: Dict[str, Any]) -> bool:
...
def verify_adversary_exists(
    session: CalderaSession,
    adversary_id: str,
) -> Dict[str, Any]:
    """Preflight check: verify that an adversary exists in Caldera and is populated."""
    result = session.request("GET", "/api/v2/adversaries")
    adversaries = as_list(result.get("json"))
    match = first_matching(adversaries, ("adversary_id", "id", "name"), [adversary_id])
    if match:
        return {
            "exists": True,
            "has_abilities": _adversary_has_abilities(match),
            "adversary_id": match.get("adversary_id") or match.get("id") or adversary_id,
            "adversary_name": match.get("name") or "",
            "atomic_ordering": match.get("atomic_ordering") or [],
        }
    return {
        "exists": False,
        "has_abilities": False,
...
def local_adversary_import_hint(plan: Dict[str, Any]) -> Dict[str, Any]:
    executor = plan.get("executor") if isinstance(plan.get("executor"), dict) else {}
    raw_path = str(
        plan.get("caldera_adversary_file")
        or executor.get("adversary_file")
        or ""
    ).strip()
    if not raw_path:
        return {}
    ...
    if not path.is_absolute():
        path = Path.cwd() / path
    return {
        "local_adversary_file": str(path),
        "local_adversary_file_exists": path.exists(),
    }
...
def select_adversary(session: CalderaSession, plan: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if match:
        adversary_id = match.get("adversary_id") or match.get("id")
        if not _adversary_has_abilities(match):
            for adv in adversaries:
                if not isinstance(adv, dict):
                    continue
                if value_matches(adv.get("name"), candidates) and _adversary_has_abilities(adv):
                    return {"adversary_id": adv.get("adversary_id") or adv.get("id")}
        return {"adversary_id": adversary_id}
...
class DispatchHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        adversary_selection = select_adversary(session, plan)
        requested_adversary_id = adversary_selection.get("adversary_id", "")
        if requested_adversary_id and requested_adversary_id != "ad-hoc":
            preflight = verify_adversary_exists(session, requested_adversary_id)
            if not preflight.get("exists"):
                self.send_json(400, {
                    "status": "adversary_not_found",
                    "error": "adversary_not_found",
                    "adversary_id": requested_adversary_id,
                    "message": f"Adversary '{requested_adversary_id}' is not imported in Caldera.",
                    **local_adversary_import_hint(plan),
                    "import_instruction": (
                        "Import it via Campaigns -> Adversaries -> Import before starting the game."
                    ),
                })
                return
            if not preflight.get("has_abilities"):
                self.send_json(400, {
                    "status": "adversary_empty",
                    "error": "adversary_empty",
                    "adversary_id": requested_adversary_id,
                    "message": (
                        f"Adversary '{requested_adversary_id}' exists but has no abilities in atomic_ordering."
                    ),
                    **local_adversary_import_hint(plan),
                    "import_instruction": (
                        "Re-import the adversary YAML or sync the Caldera resources before starting the game."
                    ),
                })
                return
        ...
        warnings = []
        launched_adversary = operation.get("adversary") if isinstance(operation.get("adversary"), dict) else {}
        launched_adversary_id = launched_adversary.get("adversary_id") or launched_adversary.get("name") or ""
        launched_adversary_name = launched_adversary.get("name") or ""
        chain_summary = summarize_chain(operation)
        used_ad_hoc = str(launched_adversary_id).strip().lower() == "ad-hoc" or str(
            launched_adversary_name
        ).strip().lower() == "ad-hoc"
        if launched_adversary_id == "ad-hoc":
            requested_id = (payload.get("adversary") or {}).get("adversary_id", "")
            warnings.append(
                f"Caldera substituted 'ad-hoc' for requested adversary '{requested_id}'. "
                "The adversary is likely not imported or has empty atomic_ordering. "
                "Import it via DEPLOYMENT.md Step 6b and restart Caldera."
            )
        if not as_list(operation.get("chain")) and not as_list(launched_adversary.get("atomic_ordering")):
            warnings.append(
                "Operation has zero abilities to execute. "
                "Check that the adversary YAML has abilities linked in atomic_ordering."
            )
        if used_ad_hoc:
            restore_results = restore_agent_groups(
                session,
            ...
            self.send_json(
                400,
                {
                    "status": "adversary_substituted",
                    "error": "adversary_substituted",
                    "message": "Caldera substituted the requested adversary with ad-hoc; refusing empty attack execution.",
                    "requested_adversary_id": requested_adversary_id,
                    "operation_id": operation_id,
                    "operation_state": operation.get("state", ""),
                    "used_ad_hoc": True,
        ...
        response = {
            "status": "dispatched",
            "operation_id": operation_id,
            "operation_status": operation.get("state", ""),
            "adversary_id": launched_adversary_id or requested_adversary_id,
            "adversary_name": launched_adversary_name or preflight.get("adversary_name", "") if requested_adversary_id and requested_adversary_id != "ad-hoc" else launched_adversary_name,
            "adversary_verified": bool(requested_adversary_id and requested_adversary_id != "ad-hoc"),
            "used_ad_hoc": used_ad_hoc,
            "abilities_ran": chain_summary.get("successful_link_count", 0),
            "links_collected": chain_summary.get("link_count", 0),
            "successful_link_count": chain_summary.get("successful_link_count", 0),
            "ability_link_count": chain_summary.get("ability_link_count", 0),
            "chain_summary": chain_summary,
            "execution_valid": not used_ad_hoc,
            "targeting": targeting,
            "operation_payload": payload,
            "launch_result": launch,
