...
from environment.network.network_model import (  # noqa: E402
    CLOUD_NODE_MAP,
    EDGE_GATEWAY_IDS,
...
def write_mulval(model, output_path, scenario_id=None):
    ...
    goals_path = output_path.parent / "goals.txt"
    attack_goal = model.get("attack_goal") or {
        "predicate": "execCode",
        "host": "cloud_db",
        "privilege": "root",
    }
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Output MulVAL .P file. For compatibility, a .json path here is "
            "treated as --json-output and the .P file is written beside it."
        ),
    )
    parser.add_argument(
        "--json-output",
    ...
    default_output = PROJECT_ROOT / "attacker" / "mulval" / "outputs" / "topology_auto.P"
    default_json_output = PROJECT_ROOT / "attacker" / "mulval" / "outputs" / "topology_auto.json"

    output_path = Path(args.output) if args.output else default_output
    json_output_path = Path(args.json_output) if args.json_output else default_json_output
    if args.output and not args.json_output and output_path.suffix.lower() == ".json":
        json_output_path = output_path
        output_path = output_path.with_suffix(".P")

    model = build_abstract_topology()
    model["scenario_id"] = args.scenario_id
    json_path = write_json(model, json_output_path)
    mulval_path = write_mulval(model, output_path, scenario_id=args.scenario_id)
    goals_path = mulval_path.parent / "goals.txt"
