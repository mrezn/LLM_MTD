...
try:
    from .game_model import evolutionary_step, normalize_population
except ImportError:
    from game_model import evolutionary_step, normalize_population  # script mode fallback
try:
    from .policy_selector import compact_selection, select_pair
except ImportError:
    from policy_selector import compact_selection, select_pair  # script mode fallback
try:
    ...
    from state_builder import (  # script mode fallback
        DEFAULT_CLOUD_METRICS_CONTAINER,
        DEFAULT_CORE_URL,
        DEFAULT_CORE_DOCKER_FALLBACK,
        DEFAULT_MTD_METRICS_URL,
...
try:
    ...
    from stage_transition import (  # script mode fallback
        DEFAULT_DECISION_TRACE_LOG,
        DEFAULT_STAGE_LOG,
        append_decision_trace,
        append_transition,
...
try:
    ...
    from strategy_manager import DEFAULT_STRATEGY_SPACE, StrategyManager  # script mode fallback
...
DOCKER_SOCKET_PATH = os.environ.get("DOCKER_HOST", "unix:///var/run/docker.sock").removeprefix(
    "unix://"
)
...
def request_json(url: str, method: str = "GET", timeout: float = 2.0) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"ok": False, "status": 0, "url": url, "json": {}, "body": "", "error": str(error)}
...
def post_json_from_container(
    ...
    if DOCKER_SOCKET_PATH and DOCKER_SOCKET_PATH.startswith("/") and not os.access(
        DOCKER_SOCKET_PATH,
        os.R_OK | os.W_OK,
    ):
        return {
            ...
            "error": (
                f"Docker socket is not readable/writable by this user: {DOCKER_SOCKET_PATH}. "
                "Run `sudo usermod -aG docker $USER && newgrp docker`, or pass "
                "`--no-cloud-policy-docker-fallback` / `--no-core-docker-fallback`."
            ),
        }
    ...
    for command in commands:
        try:
            ...
            error_str = str(error)
            attempt_errors.append(f"{command[4]} failed to start: {error_str}")
            if "permission denied" in error_str.lower():
                logger.warning(
                    "Docker socket permission denied when running '%s'. "
        ...
        if "permission denied" in detail.lower() and "docker.sock" in detail.lower():
            logger.warning(
                "Docker socket permission denied: %s. "
                "The current user is not in the 'docker' group. "
                "Fix: sudo usermod -aG docker $USER && newgrp docker",
                detail,
            )
...
def dispatch_failure_details(result: Dict[str, Any]) -> Dict[str, Any]:
    body = parse_response_json(result)
    return {
        "failure_status": str(body.get("status") or "").strip(),
        "failure_error": str(body.get("error") or result.get("error") or "").strip(),
        "failure_adversary_id": str(body.get("adversary_id") or "").strip(),
        "failure_body": body,
    }
...
def strategy_space_checksum(path: Path) -> str:
    if not path.exists():
        return ""
...
def save_population(path: Path, game_result: Dict[str, Any], stage_result: Dict[str, Any]) -> None:
    payload = {
        ...
        "global_population": game_result.get("global_population", {}),
        "last_stage": {
...
def attach_global_populations(
    game_result: Dict[str, Any],
    previous_population: Dict[str, Any],
    manager: StrategyManager,
    scenario_id: str,
) -> None:
    previous_global = previous_population.get("global_population") or {}
    global_population: Dict[str, Dict[str, float]] = {}
    for role, strategies in (
        ("attacker", manager.attacker_strategies),
        ("defender", manager.defender_strategies),
    ):
        ids = [
            str(strategy.get("id") or "")
            for strategy in strategies
            if str(strategy.get("scenario_id", "*")) in {"*", str(scenario_id)}
        ]
        prior = dict(previous_global.get(role) or previous_population.get(role) or {})
        prior.update(game_result.get(f"{role}_population") or {})
        global_population[role] = normalize_population(
            ids, prior, safe_float(manager.parameters.get("population_floor"), 0.01)
        )
    game_result["global_population"] = global_population
...
def execute_attacker(
    ...
    cloud_policy_url: str,
    timeout: float,
    filtered_out_reasons: Optional[Dict[str, Any]] = None,
    activation_diagnostics: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    plan = build_attacker_execution_plan(selection, state)
    if plan is None:
        return {
            "status": "no_active_attacker_strategy",
            "dispatch_attempted": False,
            "failure_reason": "all_attackers_filtered",
            "filtered_out_reasons": filtered_out_reasons or {},
            "attacker_activation_diagnostics": activation_diagnostics or {},
            "post_result": None,
            "remediation_hint": (
                "Enable non-strict scenario-seed attacker activation or regenerate "
                "MulVAL with exact 4-hop path"
            ),
        }
    if cloud_logger_url:
    ...
    if not execute:
        return {"status": "dry_run", "dispatch_attempted": False, "plan": plan}
    if not dispatch_url:
        return {
    ...
    result_body = response_json(result)
    execution = {
        "status": status,
        "dispatch_attempted": True,
        "plan": plan,
        "post_result": result,
        "abilities_ran": safe_int(result_body.get("abilities_ran")),
        "links_collected": safe_int(result_body.get("links_collected")),
        "successful_link_count": safe_int(result_body.get("successful_link_count")),
        "ability_link_count": safe_int(result_body.get("ability_link_count")),
        "operation_status": str(result_body.get("operation_status") or ""),
        "adversary_id": str(result_body.get("adversary_id") or ""),
        "adversary_name": str(result_body.get("adversary_name") or ""),
        "used_ad_hoc": bool(result_body.get("used_ad_hoc")),
        "adversary_verified": result_body.get("adversary_verified"),
        "execution_valid": result_body.get("execution_valid"),
        "warnings": result_body.get("warnings", []) or [],
        "chain_summary": result_body.get("chain_summary") or {},
    }
    if not result.get("ok"):
        execution.update(dispatch_failure_details(result))
    return execution
...
def wait_for_defense_commit(
    ...
    """Poll Ryu briefly so post-action state is collected after policy commit."""
    if defender_execution.get("status") != "executed":
        return {"confirmed": False, "attempts": 0, "reason": "defender_not_executed"}
    payload = defender_execution.get("payload") or {}
    expected_action = str(payload.get("action") or "")
    expected_target = str(payload.get("target") or "")
    last_status: Dict[str, Any] = {}
    for attempt in range(1, max(attempts, 1) + 1):
        result = request_json(mtd_status_url, timeout=timeout)
        last_status = result.get("json") if isinstance(result.get("json"), dict) else {}
        actions = last_status.get("active_actions") or []
        if isinstance(actions, dict):
            actions = list(actions.values())
        matched = any(
            isinstance(action, dict)
            and str(action.get("action") or "") == expected_action
            and (not expected_target or str(action.get("target") or "") == expected_target)
            for action in actions
        )
        if matched:
            return {"confirmed": True, "attempts": attempt, "status": last_status}
    ...
    return {
        ...
        "status": last_status,
        "reason": "active_policy_not_visible_before_timeout",
    }
...
def query_active_policy_count(mtd_status_url: str, timeout: float) -> Dict[str, Any]:
    status = request_json(mtd_status_url, timeout=timeout)
    status_json = status.get("json") if isinstance(status.get("json"), dict) else {}
    active_actions = status_json.get("active_actions")
    ...
    return {
        "count": count,
        "status": status,
        "known_host_diagnostics": status_json.get("known_host_diagnostics", {}),
    }
...
def stage_teardown(
    ...
    """Clear every active MTD policy before the stage to keep stages isolated."""
    if not reset_actions:
        return {
            "pre_stage_reset_attempted": False,
            "pre_stage_reset_success": False,
            "pre_stage_active_policy_count_before": 0,
            "pre_stage_active_policy_count_after": 0,
            "post_teardown_verified": False,
            "reset_mode": "disabled",
        }
    before = query_active_policy_count(mtd_status_url, timeout)
    delete_url = mtd_status_url.replace("/mtd/status", "/mtd/actions")
    delete_result = request_json(delete_url, method="DELETE", timeout=timeout)
    if not delete_result.get("ok"):
        delete_result = post_json(ryu_action_url, {
            "action": "clear_all_policies",
            "source": "stage_teardown",
        }, timeout=timeout)
    after = query_active_policy_count(mtd_status_url, timeout)
    success = bool(delete_result.get("ok")) and after["count"] == 0
    return {
        "pre_stage_reset_attempted": True,
        "pre_stage_reset_success": success,
        "pre_stage_active_policy_count_before": before["count"],
        "pre_stage_active_policy_count_after": after["count"],
        "post_teardown_verified": after["count"] == 0,
        "reset_mode": "clear_all_policies",
        "known_host_diagnostics": after.get("known_host_diagnostics", {}),
        "post_result": delete_result,
    }


def apply_no_attack_fallback(
    active: Dict[str, Any],
    manager: StrategyManager,
    state: Dict[str, Any],
) -> bool:
    """Restrict a clean, attacker-empty stage to the non-disruptive observer."""
    no_active_attack = not active.get("attacker_ids")
    if no_active_attack and safe_int(state.get("path_stage")) == 0:
        observe = [
            strategy for strategy in manager.defender_strategies
            if strategy.get("action") == "observe"
        ]
        active["defenders"] = observe
        active["defender_ids"] = [str(strategy.get("id", "")) for strategy in observe]
    return no_active_attack


def stage_persistence_policy(source_invalid: bool, no_active_attack: bool) -> Dict[str, bool]:
    """Return independent persistence decisions for learning and audit records."""
    return {
        "skip_payoff_update": no_active_attack,
        "skip_population_persistence": source_invalid or no_active_attack,
        "skip_audit_logs": source_invalid,
    }


def run_game_stage(args: argparse.Namespace) -> Dict[str, Any]:
    reset_between_stages = getattr(args, "reset_actions_between_stages", True)
    teardown_result = stage_teardown(
        args.ryu_action_url,
        args.mtd_status_url,
        args.timeout_seconds,
        reset_actions=reset_between_stages,
    )
    ...
    cloud_storage_baseline = safe_float(
        ((state.get("workload") or {}).get("cloud_storage_confirmations")),
        0.0,
    )
    overhead_baseline = state.get("overhead") or {}
    active = manager.active_lists(state, strict_preconditions=args.strict_preconditions)
    activation_diagnostics = manager.attacker_activation_diagnostics(
        state, strict_preconditions=args.strict_preconditions
    )
    activation_diagnostics.update({
        "caldera_adversaries_available": bool(args.attacker_dispatch_url),
        "alive_agents_for_entry": None,
    })
    no_active_attack = apply_no_attack_fallback(active, manager, state)
    if no_active_attack:
        defender_population = {
            str(strategy.get("id", "")): 1.0 / max(len(active["defenders"]), 1)
            for strategy in active["defenders"]
        }
        game = {
            "parameters": dict(manager.parameters),
            "attacker_population_before": {},
            "defender_population_before": defender_population,
            "attacker_population": {},
            "defender_population": defender_population,
            "attacker_average_utility": 0.0,
            "defender_average_utility": 0.0,
            "attacker_payoffs": {},
            "defender_payoffs": {},
            "attacker_utilities": {},
            "defender_utilities": {},
            "skip_payoff_update": True,
            "skip_population_persistence": True,
        }
    else:
        previous_population = load_population(
            None if args.no_population_load else args.population_file
        )
        ...
        attach_global_populations(
            game, previous_population, manager, str(state.get("scenario_id") or "")
        )
    selection_mode = (
        "safe_no_attack_fallback"
        if no_active_attack
        else (args.selection_mode or manager.parameters.get("selection_mode", "dominant"))
    )
    selection = select_pair(
        active["attackers"],
        active["defenders"],
        game,
        mode="dominant" if no_active_attack else selection_mode,
        random_seed=args.random_seed,
    )
    if no_active_attack and selection.get("defender"):
        selection["defender"].update({
            "mode": "safe_no_attack_fallback",
            "selected_by": "forced_observe_no_active_attacker",
        })
    ...
    attacker_execution = execute_attacker(
        ...
        timeout=args.timeout_seconds,
        filtered_out_reasons=(active.get("filtered_out_reasons") or {}).get("attackers", {}),
    ...
    defender_execution = execute_defender(
        ...
        execute=args.execute_defender and not no_active_attack,
    ...
    if no_active_attack:
        execution["defender"].update({
            "status": "observe_only",
            "reason": "forced_observe_no_active_attacker",
            "execution_attempted": False,
        })

    if defender_execution.get("status") == "executed" and not args.offline:
        execution["defender"]["commit_confirmation"] = wait_for_defense_commit(
            execution["defender"], args.mtd_status_url, args.timeout_seconds
        )
        game.update({
            "skip_payoff_update": True,
            "skip_population_persistence": True,
            "selection_mode": "safe_no_attack_fallback",
        })

    if not args.cloud_logger_url and not args.no_auto_defense_event:
        logger.warning(
            "STRATEGY_CLOUD_LOGGER_URL is empty; defense event posting is disabled. "
            "Ryu evidence will still be used, but dashboard/cloud_logger evidence may be incomplete."
        )
    ...
    if not args.no_observe_next_state:
        ...
        next_constraints = {
            **constraints,
            ...
            "attacker_execution": attacker_execution,
            "flow_rules_installed_baseline": safe_int(
                overhead_baseline.get(
                    "flow_rules_installed_total", overhead_baseline.get("flow_rules_installed")
                )
            ),
            "meters_added_baseline": safe_int(
                overhead_baseline.get("meters_added_total", overhead_baseline.get("meters_added"))
            ),
        }
    ...
    metadata = {
        "strategy_space_checksum": strategy_space_checksum(args.strategy_space),
        ...
        "attacker_activation_diagnostics": activation_diagnostics,
        "pre_stage_reset": teardown_result,
    }
    transition.update(metadata)
    decision_trace.update(metadata)

    result = {
        "schema_version": "llm-mtd-strategy-stage-result-v1",
        "strategy_space_checksum": metadata["strategy_space_checksum"],
        "state": state,
        ...
        "stage_teardown": teardown_result,
    }

    skip_persistence = should_skip_persistence_for_state(state)
    persistence_policy = stage_persistence_policy(skip_persistence, no_active_attack)
    skip_population_persistence = persistence_policy["skip_population_persistence"]
    result["persistence"] = {
        ...
        "skip_payoff_update": persistence_policy["skip_payoff_update"],
        "skip_population_persistence": skip_population_persistence,
        "stage_logged": False,
        "decision_trace_logged": False,
    }

    if not args.no_save_population and not skip_population_persistence:
        save_population(args.population_file, game, result)
        result["persistence"]["population_saved"] = True
    if not args.no_stage_log and not persistence_policy["skip_audit_logs"]:
        transition = append_transition(args.stage_log, transition)
        result["transition"] = transition
        result["persistence"]["stage_logged"] = True
    if not args.no_decision_trace_log and not persistence_policy["skip_audit_logs"]:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Allow scenario-seed path prefixes when an exact MulVAL path is absent (default in non-strict mode).",
    )
    parser.add_argument(
        "--explain-attacker-filter",
        action="store_true",
        help="Include candidate-level attacker activation diagnostics in the JSON result.",
    )
    ...
    parser.add_argument("--selection-mode", choices=["dominant", "sample", "best_utility"], default="")
    ...
    parser.add_argument("--execute-attacker", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--attacker-dispatch-url", default=DEFAULT_ATTACKER_DISPATCH_URL)
    parser.add_argument("--execute-defender", action=argparse.BooleanOptionalAction, default=False)
    ...
    parser.add_argument(
        "--reset-actions-between-stages",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Clear previous stage's Ryu policy actions before running the next stage.",
    )
    return parser
