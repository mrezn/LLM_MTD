...
class StrategyManager:
    ...
    def active_attackers(
        ...
        return [
            ...
            if self._activation_failure_reason(
                strategy,
                state,
                role="attacker",
                strict_preconditions=strict,
            ) is None
        ]
    ...
    def active_lists(
        ...
        strict = self._strict_preconditions(state, strict_preconditions)
        attackers = self.active_attackers(state, strict_preconditions=strict)
        defender_state = dict(state)
        defender_state["active_attacker_available"] = bool(attackers)
        defender_state["path_progress_observed"] = safe_float(state.get("path_stage")) >= 1
        defenders = self.active_defenders(defender_state, strict_preconditions=strict)
        filtered_out_reasons = {
            "attackers": {
                strategy_id(strategy): reason
                for strategy in self.attacker_strategies
                for reason in [self._activation_failure_reason(
                    strategy,
                    defender_state,
                    role="attacker",
                    strict_preconditions=strict,
        ...
        return {
            ...
            "attacker_filter_mode": "strict_mulval_exact" if strict else "non_strict_scenario_seed",
            "scenario_seed_allowed": not strict,
        }
    ...
    def _activation_failure_reason(
        ...
        if role == "attacker" and not self._mulval_allows_attacker(
            strategy, state, strict_preconditions
        ):
            if strict_preconditions and not bool_value(
                (state.get("mulval") or {}).get("mulval_exact_match_found")
            ):
                return "missing_exact_mulval_path_strict"
            return "mulval_disallowed"
    ...
    def _activation_matches(
        ...
        required_any = activation.get("required_any_flags", [])
        if required_any and not any(
            bool_value(get_state_value(state, str(flag), False)) for flag in required_any
        ):
            return False
    ...
    def _scenario_seed_allows_attacker(
        self,
        strategy: Dict[str, Any],
        state: Dict[str, Any],
    ) -> bool:
        strategy_path = self._strategy_path(strategy)
        seed_path = [str(item) for item in (state.get("current_path") or [])]
        if not strategy_path or not seed_path:
            return False
        return strategy_path == seed_path[:len(strategy_path)]
    ...
    def _mulval_allows_attacker(
        ...
        exact_found = bool_value(mulval.get("mulval_exact_match_found"))
        if strict_preconditions and not exact_found:
            return False

        plausible = self._strategy_path_is_plausible(strategy, state)
        if not plausible:
            if strict_preconditions or not self._scenario_seed_allows_attacker(strategy, state):
                return False
    ...
    def attacker_activation_diagnostics(
        self,
        state: Dict[str, Any],
        strict_preconditions: Optional[bool] = None,
    ) -> Dict[str, Any]:
        ...
        for strategy in self.attacker_strategies:
            sid = strategy_id(strategy)
            scenario_ok = self._scenario_matches(strategy, state)
            activation = strategy.get("activation") or {}
            stage = safe_float(state.get("path_stage"))
            path_stage_ok = (
                safe_float(activation.get("min_path_stage", 0.0))
                <= stage
                <= safe_float(activation.get("max_path_stage", 99.0))
            )
            mulval_ok = self._strategy_path_is_plausible(strategy, state)
            scenario_seed_ok = self._scenario_seed_allows_attacker(strategy, state)
            reason = self._activation_failure_reason(
                strategy, state, role="attacker", strict_preconditions=strict
            )
            if reason is None and scenario_seed_ok and not mulval_ok:
                reason = "allowed_by_scenario_seed_non_strict"
            candidates[sid] = {
                "scenario_ok": scenario_ok,
                "path_stage_ok": path_stage_ok,
                "mulval_ok": mulval_ok,
                "scenario_seed_ok": scenario_seed_ok,
                "final_active": self._activation_failure_reason(
                    strategy, state, role="attacker", strict_preconditions=strict
                ) is None,
        ...
        return {
            ...
            "mulval_exact_match_required": strict,
            "mulval_exact_match_found": bool_value(mulval.get("mulval_exact_match_found")),
            "mulval_path_mismatch": bool_value(mulval.get("mulval_path_mismatch")),
            "mulval_match_type": mulval.get("mulval_match_type", ""),
            "attacker_filter_mode": "strict_mulval_exact" if strict else "non_strict_scenario_seed",
            "scenario_seed_allowed": not strict,
            "scenario_seed_path_found": bool(state.get("current_path")),
            "candidate_attackers": candidates,
        }
