...
DEFAULT_SCENARIO_REGISTRY = Path(__file__).resolve().parents[1] / "attacker" / "scenarios" / "attack_scenarios.json"
DEFAULT_MULVAL_POLICY = (
    Path(__file__).resolve().parents[1] / "attacker" / "mulval" / "outputs" / "base_edge2_policy.json"
)
...
def fetch_json_from_container(
    ...
    if DOCKER_SOCKET_PATH and DOCKER_SOCKET_PATH.startswith("/") and not os.access(
        ...
        errors.append(
            f"docker json fetch skipped for {container_name}:{path}: "
...
def current_path_risk_details(
    path: List[str],
    plausible_candidates: List[Dict[str, Any]],
) -> Dict[str, Any]:
    default_risk = 0.50
    mulval_candidates = [
        candidate for candidate in plausible_candidates
        if str(candidate.get("source") or "").startswith("mulval_")
    ]
    if not path:
        return {
            "current_path_risk": default_risk,
            "mulval_path_mismatch": True,
            "mulval_match_type": "missing_game_path",
            "matched_candidate": {},
        }

    for candidate in mulval_candidates:
        if candidate.get("path") == path:
            return {
                "current_path_risk": safe_float(candidate.get("risk_score"), default_risk),
                "mulval_path_mismatch": False,
                "mulval_match_type": "exact_path",
...
def query_ryu_defense_state(
    mtd_status_data: Optional[Dict[str, Any]] = None,
    controller_metrics: Optional[Dict[str, Any]] = None,
    defense_metrics: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    """Derive defense flags from live Ryu state, with stale metric detection."""
    if not isinstance(mtd_status_data, dict):
        mtd_status_data = {}
    controller_metrics = controller_metrics if isinstance(controller_metrics, dict) else {}
    defense_metrics = defense_metrics if isinstance(defense_metrics, dict) else {}

    active_actions_raw = mtd_status_data.get("active_actions")
    if isinstance(active_actions_raw, dict):
        active_actions_list = list(active_actions_raw.values())
    elif isinstance(active_actions_raw, list):
        active_actions_list = active_actions_raw
    else:
        active_actions_list = []

    action_names = []
    active_policy_actions = []
    active_mtd_flow_count = 0
    active_mtd_meter_count = 0
    for action in active_actions_list:
        ...
        if action_name:
            ...
            active_policy_actions.append(
                f"{action_name}:{target}" if target else action_name
            )
            active_mtd_flow_count += safe_int(action.get("flow_rules_installed"))
            active_mtd_meter_count += safe_int(action.get("meters_added"))
    ...
    reported_rate_limit_active = bool_metric(defense_metrics.get("rate_limit_active"))
    reported_defense_active = (
        reported_defense_success
        or reported_drop_rules_active
        or reported_counters_stopped
        or reported_rate_limit_active
    )

    live_drop_rules_active = any(
        action in {"quarantine_sensor", "isolate_sensor", "clear_quarantine"}
        or "quarantine" in action
        or "isolate" in action
        for action in action_names
    )
    live_rate_limit_active = any("rate_limit" in action for action in action_names)
    live_counters_stopped = any("isolate" in action for action in action_names)
    controller_has_active_defense = bool(active_actions_list)
    stale_defense_metrics = controller_has_active_defense is False and reported_defense_active

    return {
        "drop_rules_active": live_drop_rules_active or (
            reported_drop_rules_active and not stale_defense_metrics
        ),
        "rate_limit_active": live_rate_limit_active or (
            reported_rate_limit_active and not stale_defense_metrics
        ),
        "counters_stopped": live_counters_stopped or (
            reported_counters_stopped and not stale_defense_metrics
        ),
        ...
        "active_mtd_flow_count": active_mtd_flow_count,
        "active_mtd_meter_count": active_mtd_meter_count,
        "flow_rules_installed": safe_int(
            controller_metrics.get("ryu_controller_flow_rules_installed_total")
        ),
        "flow_rules_deleted": safe_int(
        ...
        "ryu_drop_rules_active": live_drop_rules_active,
        "ryu_rate_limit_active": live_rate_limit_active,
        "ryu_counters_stopped": live_counters_stopped,
        "stale_defense_metrics": stale_defense_metrics,
        "defense_evidence_source": "ryu_status" if active_actions_list else (
            "defense_metrics" if reported_defense_active else "none"
        ),
    }


def derive_cloud_seen(
    *,
    attack_metrics: Dict[str, Any],
    cloud_storage_current: float,
    constraints: Optional[Dict[str, Any]],
    has_attack_context: bool,
) -> Dict[str, Any]:
    constraints = constraints or {}
    cloud_storage_baseline = safe_float(constraints.get("cloud_storage_baseline"), 0.0)
    cloud_storage_min_delta = safe_float(constraints.get("cloud_storage_min_delta"), 5.0)
    cloud_storage_min_ratio = safe_float(constraints.get("cloud_storage_min_ratio"), 0.05)
    attacker_exec = constraints.get("attacker_execution")
    if not isinstance(attacker_exec, dict):
        attacker_exec = {}
    abilities_ran = safe_int(attacker_exec.get("abilities_ran", 0))
    if abilities_ran <= 0:
        abilities_ran = safe_int((attacker_exec.get("post_result") or {}).get("abilities_ran", 0))

    cloud_storage_delta = max(cloud_storage_current - cloud_storage_baseline, 0.0)
    cloud_storage_delta_ratio = (
        cloud_storage_delta / cloud_storage_baseline
        if cloud_storage_baseline > 0
        else 0.0
    )
    cloud_exfil_confirmations = safe_int(
        attack_metrics.get("cloud_exfil_confirmations_total"),
        safe_int(attack_metrics.get("cloud_exfil_confirmations")),
    )
    explicit_cloud_seen = bool_metric(attack_metrics.get("cloud_seen"))
    summary_rate_changes = bool_metric(attack_metrics.get("cloud_summary_rate_changes"))
    attack_success_signal = bool_metric(attack_metrics.get("attack_effect_success")) or bool_metric(
        attack_metrics.get("success")
    )

    cloud_seen = False
    cloud_seen_reason = "no_attack_correlated_cloud_signal"
    cloud_seen_confidence = 0.10
    if cloud_exfil_confirmations > 0:
        cloud_seen = True
        cloud_seen_reason = "cloud_exfil_confirmations"
        cloud_seen_confidence = 0.98
    elif explicit_cloud_seen and (abilities_ran > 0 or attack_success_signal):
        cloud_seen = True
        cloud_seen_reason = "attack_metric_cloud_seen"
        cloud_seen_confidence = 0.90
    elif summary_rate_changes and abilities_ran > 0 and has_attack_context:
        cloud_seen = True
        cloud_seen_reason = "cloud_summary_rate_changes"
        cloud_seen_confidence = 0.78
    elif (
        has_attack_context
        and abilities_ran > 0
        and cloud_storage_baseline > 0
        and cloud_storage_delta >= cloud_storage_min_delta
        and cloud_storage_delta_ratio >= cloud_storage_min_ratio
    ):
        cloud_seen = True
        cloud_seen_reason = "cloud_storage_delta_threshold"
        cloud_seen_confidence = 0.70
    elif cloud_storage_baseline <= 0:
        cloud_seen_reason = "missing_cloud_storage_baseline"
    elif abilities_ran <= 0:
        cloud_seen_reason = "no_caldera_abilities_ran"
    elif cloud_storage_delta < cloud_storage_min_delta:
        cloud_seen_reason = "cloud_storage_delta_below_absolute_threshold"
    elif cloud_storage_delta_ratio < cloud_storage_min_ratio:
        cloud_seen_reason = "cloud_storage_delta_below_relative_threshold"

    return {
        "cloud_seen": cloud_seen,
        "cloud_seen_reason": cloud_seen_reason,
        "cloud_seen_confidence": cloud_seen_confidence,
        "cloud_storage_baseline": cloud_storage_baseline,
        "cloud_storage_current": cloud_storage_current,
        "cloud_storage_delta": cloud_storage_delta,
        "cloud_storage_delta_ratio": cloud_storage_delta_ratio,
        "cloud_exfil_confirmations": cloud_exfil_confirmations,
        "caldera_abilities_ran": abilities_ran,
    }


def derive_effective_path_stage(
    raw_path_stage: int,
    drop_rules_active: bool,
    counters_stopped: bool,
    ...
    regression = 0
    reasons: List[str] = []
    if drop_rules_active:
        regression = max(regression, 1)
        reasons.append("drop_rules_active")
    if counters_stopped:
        regression = max(regression, 2)
        reasons.append("counters_stopped")
    if defense_success and not attack_effect_success and raw_path_stage > 0:
        regression = max(regression, 1)
        reasons.append("defense_success_without_attack_effect")
    effective = max(0, raw_path_stage - regression)
    if effective >= raw_path_stage:
        return effective, "blocking_active_no_regression" if reasons else "none"
    return effective, ",".join(reasons) or "none"
...
def build_state(
    ...
    risk_details = current_path_risk_details(path, plausible_candidates)
    current_path_risk = risk_details["current_path_risk"]
    if risk_details["mulval_path_mismatch"]:
        notes.append(
            f"game seed path {path_key(path)} has no exact match in MulVAL "
            f"path_risk_scores; using conservative default risk={current_path_risk:.2f}"
        )
        if bool_metric((constraints or {}).get("strict_preconditions")):
            raise RuntimeError(
                f"MulVAL exact path match missing for {path_key(path)} under strict_preconditions"
            )
    ...
    cloud_evidence = derive_cloud_seen(
        attack_metrics=attack_metrics,
        cloud_storage_current=cloud_storage_confirmations,
        constraints=constraints,
        has_attack_context=has_attack_context,
    )
    cloud_seen = bool(cloud_evidence["cloud_seen"])
    ...
    raw_path_stage = 3 if cloud_seen else 2 if worker_seen else 1 if gateway_seen else 0
    ...
    meters_added = safe_int(controller_metrics.get("ryu_controller_meters_added_total"))
    flow_rules_baseline_raw = (constraints or {}).get("flow_rules_installed_baseline")
    meters_added_baseline_raw = (constraints or {}).get("meters_added_baseline")
    flow_rules_installed_delta = (
        max(flow_rules_installed - safe_int(flow_rules_baseline_raw), 0)
        if flow_rules_baseline_raw is not None
        else 0
    )
    meters_added_delta = (
        max(meters_added - safe_int(meters_added_baseline_raw), 0)
        if meters_added_baseline_raw is not None
        else 0
    )
    controller_reachable = bool(mtd_metrics_text or mtd_status_data)

    defense_state = query_ryu_defense_state(
        mtd_status_data=mtd_status_data,
        controller_metrics=controller_metrics,
        defense_metrics=defense_metrics,
    )
    stale_defense_metrics = bool(defense_state.get("stale_defense_metrics"))
    ...
    defense_success = bool(defense_state.get("defense_success"))
    drop_rules_active = bool(defense_state.get("drop_rules_active"))
    rate_limit_active = bool(defense_state.get("rate_limit_active"))
    counters_stopped = bool(defense_state.get("counters_stopped"))
    defense_active = bool(defense_state.get("defense_active"))
    effective_path_stage, path_regression_reason = derive_effective_path_stage(
        raw_path_stage=raw_path_stage,
        drop_rules_active=drop_rules_active,
        counters_stopped=counters_stopped,
        defense_success=defense_success,
        attack_effect_success=attack_effect_success,
    )
    path_stage = effective_path_stage
    ...
    return {
        "schema_version": "llm-mtd-game-state-v2",
        ...
        "raw_path_stage": raw_path_stage,
        "effective_path_stage": effective_path_stage,
        "path_regression_reason": path_regression_reason,
        "mulval_exact_match_required": bool_metric(
            operational_constraints.get("strict_preconditions")
        ),
        "mulval_exact_match_found": not risk_details["mulval_path_mismatch"],
        "mulval_path_mismatch": risk_details["mulval_path_mismatch"],
        "mulval_match_type": risk_details["mulval_match_type"],
        "attacker_filter_mode": (
            "strict_mulval_exact"
            if bool_metric(operational_constraints.get("strict_preconditions"))
            else "non_strict_scenario_seed"
        ),
        "scenario_seed_allowed": not bool_metric(
            operational_constraints.get("strict_preconditions")
        ),
        "mulval": {
        ...
        "path_evidence": {
            ...
            "cloud_seen": cloud_seen,
            "cloud_seen_reason": cloud_evidence["cloud_seen_reason"],
            "cloud_seen_confidence": cloud_evidence["cloud_seen_confidence"],
            "raw_path_stage": raw_path_stage,
            "effective_path_stage": effective_path_stage,
            "path_regression_reason": path_regression_reason,
        },
        "defense_evidence": {
            ...
            "rate_limit_active": rate_limit_active,
            "counters_stopped": counters_stopped,
            "active_policy_actions": defense_state.get("active_policy_actions", []),
            "flow_rules_installed": defense_state.get("flow_rules_installed", 0),
            "flow_rules_deleted": defense_state.get("flow_rules_deleted", 0),
            "meters_added": defense_state.get("meters_added", 0),
            "ryu_drop_rules_active": defense_state.get("ryu_drop_rules_active", False),
            "ryu_counters_stopped": defense_state.get("ryu_counters_stopped", False),
            "stale_defense_metrics": stale_defense_metrics,
            "source": defense_state.get("defense_evidence_source", "none"),
        },
        "observability": {
        ...
        "workload": {
            ...
            "cloud_storage_baseline": cloud_evidence["cloud_storage_baseline"],
            "cloud_storage_delta": cloud_evidence["cloud_storage_delta"],
            "cloud_storage_delta_ratio": cloud_evidence["cloud_storage_delta_ratio"],
            "cloud_exfil_confirmations": cloud_evidence["cloud_exfil_confirmations"],
        },
        "overhead": {
            "controller_apply_ms": controller_apply_ms,
            "flow_rules_installed": flow_rules_installed,
            "flow_rules_installed_total": flow_rules_installed,
            "flow_rules_installed_delta": flow_rules_installed_delta,
            "active_mtd_flow_count": defense_state.get("active_mtd_flow_count", 0),
            "flow_delete_commands": flow_delete_commands,
            "meters_added": meters_added,
            "meters_added_total": meters_added,
            "meters_added_delta": meters_added_delta,
            "active_mtd_meter_count": defense_state.get("active_mtd_meter_count", 0),
