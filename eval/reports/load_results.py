...
def load_result_frames(
    *,
    ...
    stage_df = _merge_stage_summaries(stage_df, summary_df)
...
def _merge_stage_summaries(stage_df: pd.DataFrame, summary_df: pd.DataFrame) -> pd.DataFrame:
    """Fill report fields from the canonical stage-summary stream by stage identity."""
    if summary_df.empty:
        return stage_df
    if stage_df.empty:
        return summary_df.copy()
    keys = [key for key in ("scenario_id", "stage_id") if key in stage_df.columns and key in summary_df.columns]
    if len(keys) != 2:
        return stage_df
    merged = stage_df.merge(summary_df, on=keys, how="left", suffixes=("", "__summary"))
    for column in summary_df.columns:
        summary_column = f"{column}__summary"
        if summary_column not in merged.columns:
            continue
        if column not in merged.columns:
            ...
            if merged[column].dtype == object:
                missing = missing | merged[column].map(lambda value: value == "" or value == [] or value == {})
            elif merged[summary_column].dtype == object:
                merged[column] = merged[column].astype(object)
            merged.loc[missing, column] = merged.loc[missing, summary_column]
...
def _flatten_stage_row(
    ...
    *,
    ...
    expected_effects = list(
        stage_validation.get("expected_defense_effects", [])
        or selection_defender.get("expected_effects", [])
        or []
    )
    ...
    post_ok = _coerce_bool(_nested_get(execution_defender, "post_result", "ok"), False)
    ryu_installed = bool(
        _as_float(ryu_response.get("active_policy_actions"), ryu_body.get("active_policy_actions"), 0) > 0
        or _as_float(ryu_response.get("flow_rules_installed"), ryu_body.get("flow_rules_installed"), 0) > 0
        or _as_float(ryu_response.get("meters_added"), ryu_body.get("meters_added"), 0) > 0
    )
    inferred_confirmation = bool(defense_status == "executed" and post_ok and ryu_installed)
...
def _flatten_summary_row(row: dict[str, Any], *, method: str) -> dict[str, Any]:
    ...
    return {
        ...
        "llm_latency_ms": _as_float(llm.get("latency_ms"), llm.get("llm_latency_ms")),
        "llm_request_success": _coerce_optional_bool(llm.get("request_success"), stage_validation.get("llm_request_success")),
        "llm_parse_success": _coerce_optional_bool(llm.get("parse_success"), stage_validation.get("llm_parse_success")),
        "llm_timeout_failed": _coerce_bool(llm.get("timeout"), llm.get("llm_timeout"), False),
        "llm_reasoning_summary": llm.get("reasoning_summary", ""),
        ...
        "llm_baseline_alignment": str(
            llm.get("baseline_alignment")
            or llm.get("final_decision_alignment_vs_baseline")
            or "unknown"
        ),
        "llm_override_reason": str(llm.get("override_reason") or ""),
        "llm_ranked_candidates": (
            llm.get("candidate_rankings")
            or llm.get("llm_ranked_candidates")
            or row.get("candidate_rankings")
            or []
        ),
