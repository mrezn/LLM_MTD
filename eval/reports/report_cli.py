...
def build_report(
    *,
    ...
    project_root = next(
        (
            candidate for candidate in eval_stage_history.resolve().parents
            if (candidate / "game").is_dir() and (candidate / "eval").is_dir()
        ),
        eval_stage_history.resolve().parents[1],
    )
    default_summaries = project_root / "outputs" / "raw" / "stage_summaries.jsonl"
    if eval_stage_summaries is None and default_summaries.exists():
        eval_stage_summaries = default_summaries
    if baseline_stage_history is None and eval_stage_history.parent.name == "game":
        baseline_stage_history = eval_stage_history
        live_history = project_root / "outputs" / "raw" / "live_stage_history.jsonl"
        if live_history.exists():
            eval_stage_history = live_history
    ...
    manifest = {
        ...
        "include_debug_stages": include_debug_stages,
    }
...
def _infer_emo_root(baseline_stage_history: Path | None, eval_stage_history: Path) -> Path:
    if baseline_stage_history is not None:
        baseline_path = baseline_stage_history.resolve()
        for candidate in baseline_path.parents:
            if _is_environment_root(candidate):
                return candidate

    eval_path = eval_stage_history.resolve()
    for candidate in eval_path.parents:
        if _is_environment_root(candidate):
            return candidate

    sibling = eval_path.parent.parent.parent / "LLM_MTD_emo"
    if sibling.exists():
        return sibling
    raise RuntimeError(
        "Unable to locate the environment sources. Run from the modular project or pass "
        "--baseline-stage-history from a legacy LLM_MTD_emo repository."
    )


def _is_environment_root(path: Path) -> bool:
    legacy_layout = (path / "network_model.py").is_file()
    modular_layout = (path / "environment" / "network" / "network_model.py").is_file()
    return legacy_layout or modular_layout
