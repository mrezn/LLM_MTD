...
def build_scenario_attack_setup_table(
    ...
    *,
    ...
    scenarios_path = _first_existing_path(
        emo_root / "attacker" / "scenarios" / "attack_scenarios.json",
        emo_root / "integrations" / "attack_scenarios.json",
    )
    strategy_space_path = _first_existing_path(
        emo_root / "game" / "strategy_space.json",
        emo_root / "integrations" / "strategy" / "strategy_space.json",
    )
...
def _canonical_case_study_summary(row: pd.Series) -> str:
    ...
    comparable = bool(row.get("comparable_stage"))
    path_regressed = bool(row.get("path_regressed"))
    defense_executed = bool(row.get("defense_executed"))
    attack_active = bool(row.get("attack_active"))
    if not comparable:
        outcome = "no comparable attack stage"
    elif confirmed and path_regressed:
        outcome = "attack pressure reduced"
    elif defense_executed and not confirmed:
        outcome = "defense executed but not confirmed"
    elif attack_effect_success and not defense_success:
...
def _load_network_model(emo_root: Path) -> Any:
    module_path = _first_existing_path(
        emo_root / "environment" / "network" / "network_model.py",
        emo_root / "network_model.py",
    )
...
def _first_existing_path(*candidates: Path) -> Path:
    ...
    raise FileNotFoundError(f"None of the expected report inputs exist: {expected}")
